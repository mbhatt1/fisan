...
def create_model(img_rows, img_cols):
    model = Sequential()
    model.add(Convolution2D(4, 3, 3, border_mode='same', init='glorot_normal',
                            input_shape=(1, img_rows, img_cols)))
    model.add(MaxPooling2D(pool_size=(4,4)))
    model.add(Dropout(0.15))

    model.add(Convolution2D(8, 3, 3, border_mode='same', init='glorot_normal'))
    model.add(Convolution2D(8, 3, 3, border_mode='same', init='glorot_normal'))
    model.add(Convolution2D(8, 3, 3, border_mode='same', init='glorot_normal'))
    model.add(MaxPooling2D(pool_size=(4,4)))
    model.add(Dropout(0.15))
    ...
    rms= RMSprop(lr=0.0001, rho=0.9, epsilon=0)
    adam= Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08)
    adagard = Adagrad(lr=0.01, epsilon=1e-08)
    nadam = Nadam(lr=0.01, beta_1=0.9, beta_2=0.999, epsilon=1e-08, schedule_decay=0.001)
